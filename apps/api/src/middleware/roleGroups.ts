...
export const OFFICE_ROLES: Role[] = [
  ...
  Role.STAFF,
  Role.OPERATIONS_HR,
  Role.OPERATIONS_FRONT_DESK,
];

/** Creating / completing a linked Student profile (leadership + HR). */
export const STUDENT_CREATE_ROLES: Role[] = [...LEADERSHIP_ROLES, Role.OPERATIONS_HR];
...
export const COURSE_STAFF_ROLES: Role[] = [...OFFICE_ROLES, Role.OPERATIONS_LECTURER];

/** Submit coursework (enrolled students only; guests use future workshop/library routes). */
export const ASSIGNMENT_SUBMIT_ROLES: Role[] = [Role.PRESENT_STUDENT];

/** View another user’s profile (directory / admin). */
export const USER_PROFILE_VIEWERS: Role[] = [...LEADERSHIP_ROLES, Role.OPERATIONS_LECTURER];
